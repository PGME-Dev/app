...
class _PdfTextRenderBox extends RenderBox with PdfPageTextSelectable, Selectable, SelectionRegistrant {
  ...
  SelectionResult dispatchSelectionEvent(SelectionEvent event) {
    // PATCH (pgme/app vendor): removed the
    //   if (!_textWidget._state.widget.enabled) return SelectionResult.none;
    // guard. Upstream pdfrx 1.3.5 disables selection events whenever it
    // considers an InteractiveViewer interaction "ongoing" (any touch +
    // 300ms grace timer). On a zoomed-in page even tiny finger movement
    // looks like the start of a pan to ScaleGestureRecognizer, so the
    // overlay drops every selection event and the user cannot select
    // text. Always processing the event lets SelectionArea's long-press
    // and drag work regardless of zoom level.
